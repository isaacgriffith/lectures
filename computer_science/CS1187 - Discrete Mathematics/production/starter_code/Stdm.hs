...
which was published by Springer in January, 2000.

The software is written in Haskell 98, the standard functional
programming langauge. It runs interactively under the GHCi implementation.
Under Linux, you can start ghci witht he command:

$ ghci

The original version of this software, and its documentation, can be downloaded
from the book web page:

www.dcs.gla.ac.uk/~jtod/discrete-mathematics/

It's a good idea to check the web page periodically to keep the software
up to date.

This program is a script. Each line is Haskell source code that can be executed
-}
...
{- 
-- Proof Checker for Propositional Logic Data Structures 
-}
data Prop
  = FALSE
  | TRUE
...
{-
When a proof is checked, several pieces of information are gathered together
into a data structure called Status
-}
type Status =
...
{-
-- Set, Functions, and Relations Data Types
-}
type Set a = [a]
...
--
-- Mini-tutorial on Using the Proof Checker for Propositional Logic
--
-- And Intoduction:
-- ~~~~~~~~~~~~~~~~
--
...
check_proof_with_db db (Theorem thas thc) proof =
  do let (valid, ms, uda, concl) = traverse' db proof
...
{-
The real work is performed in the traverse function, which has a separate
case for checking each inference rule. Nearly all the complexity in the
following code results from an attempt to provide meaningful error messages;
if the aim were merely to decide whether a proof is valid, then the implementation
could rely much more on Haskell's pattern matching mechanism, and everything
...
traverse' db (AndI (a,b) c) =
  let (avalid, amsgs, auda, aconcl) = traverse' db a
      (bvalid, bmsgs, buda, bconcl) = traverse' db b
      (ok, msg) =
        case c of
...
traverse' db (OrE (a,b,c) d) =
  let (avalid, amsgs, auda, aconcl) = traverse' db a
      (bvalid, bmsgs, buda, bconcl) = traverse' db b
      (cvalid, cmsgs, cuda, cconcl) = traverse' db c
      (ok,msg,uda) =
        case aconcl of
